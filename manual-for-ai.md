...
`artifacts/db/migrations/NNNN_<desc>.sql`. Not a separate command — runs after
validate, before backend/frontend codegen.
...
- First run (no snapshot) → `0001_initial.sql` with all CREATE TABLE/INDEX/FK.
  Subsequent runs emit only the diff as ALTER statements. No change → no file.
