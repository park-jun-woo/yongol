...
Polarity: `true` triggers the guard (`IsZeroBalance` → true → 402).

### Function-level annotations

| Annotation | Purpose |
|---|---|
| `// @no-pagination` | Exempts list endpoints from S-63. |
| `// @state-neutral` | Declares state-independent operation. Exempts from XSM-27. |

`@auth` `ResourceID` must be `string` — use `request.id` (path param), not `wf.ID` (pgtype.UUID) (XFS-70).
`@state` inputs must be `string` — use `wf.Status` (TEXT column), not `wf.ID` (XSM-71).
...
`@post`/`@put` Inputs must use dotted references (`currentUser.Email`, `request.title`), not standalone reserved sources (`currentUser`, `request`). `@call` is exempt.
