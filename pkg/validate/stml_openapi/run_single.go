...
// runSingle executes all STML<->OpenAPI cross-validation rules for a single-site
// project. OpenAPIDoc nil makes cross-validation impossible, so that branch
// early-returns. STML 0 pages is no longer an early-return when the frontend is
// ON: the page loop becomes a no-op while the coverage rules (XMO-10/11/12)
// still run to close the gozhip gap.
func runSingle(fs *yongol.Fullstack) []diagnostic.Diagnostic {
	...
	for _, page := range fs.STMLPages {
		...
		for _, a := range page.Actions {
			...
			diags = append(diags, tm57MutationRedirectRequired(a, page.FileName, opMap)...)
		}
