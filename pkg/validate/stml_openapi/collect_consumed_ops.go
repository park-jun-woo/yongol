...
//ff:what collectConsumedOps — STML data-fetch/data-action + 레이아웃 data-logout + 컴포넌트 .tsx의 api 호출에서 소비된 operationId 집합 수집
...
// and a stale no-front tag on it would escape XMO-12), plus
// api.<operationId>( calls inside referenced components' .tsx files under
...
func collectConsumedOps(pages []stml.PageSpec, layouts []stml.LayoutSpec, specsDir string, ops map[string]struct{}) map[string]struct{} {
