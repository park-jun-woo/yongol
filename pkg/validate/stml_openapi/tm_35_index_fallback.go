...
//ff:what TM-35 — frontend ON 인데 인덱스 미선언("/" 미점유 + frontend.index 없음) — 파일명 정렬 폴백 가시화 (WARNING)
...
// (data-route) and manifest.frontend.index is absent, so the emitter's
...
func tm35IndexFallback(fs *yongol.Fullstack, opMap map[string]operationEntry) []diagnostic.Diagnostic {
	...
	return []diagnostic.Diagnostic{{
		...
		Advice:  "Declare the index: set manifest frontend.index to an STML page name (\"/\" redirects there), or mount a page at \"/\" with data-route=\"/\"",
	}}
}
