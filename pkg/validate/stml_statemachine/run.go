...
//ff:what Run — STML↔stateDiagram 교차 검증 실행 (TM-15, TM-18). 한쪽이라도 없으면 no-op.
...
// Run executes all STML↔stateDiagram cross-validation rules (TM-15, TM-18). It
// is a no-op unless both STML pages and stateDiagrams are present, so projects
// without a stateDiagram see no change in validation output.
