...
//ff:what S-2 — s02GetResult 검증 (Result nil → ERROR, Result 있음 → 통과, 비 get 스킵)
...
func TestS02GetResult(t *testing.T) {
	t.Run("fires", func(t *testing.T) {
		...
		diags := s02GetResult(fs)
		if len(diags) != 1 {
			t.Fatalf("got %d diags, want 1", len(diags))
		}
		if !strings.Contains(diags[0].Message, "[S-2]") {
			t.Errorf("Message = %q, want [S-2]", diags[0].Message)
		}
	})
