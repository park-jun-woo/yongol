...
func TestS64EmptyExistsModelOnly(t *testing.T) {
	...
	t.Run("Skips_nil_ground", func(t *testing.T) {
		assertNoDiag(t, s64EmptyExistsModelOnly(&yongol.Fullstack{}), "[S-64]")
	})
	t.Run("Skips_empty_target", func(t *testing.T) {
		fs := buildS64Fixture("X",
			[]parsessac.Sequence{{Type: parsessac.SeqEmpty, Line: 3, Target: ""}},
			nil, nil, nil)
		assertNoDiag(t, s64EmptyExistsModelOnly(fs), "[S-64]")
	})
	t.Run("Skips_implicit_var", func(t *testing.T) {
		fs := buildS64Fixture("X",
			[]parsessac.Sequence{{Type: parsessac.SeqEmpty, Line: 3, Target: "request"}},
			nil, nil, nil)
		assertNoDiag(t, s64EmptyExistsModelOnly(fs), "[S-64]")
	})
	t.Run("Skips_unknown_var_type", func(t *testing.T) {
		fs := buildS64Fixture("X",
			[]parsessac.Sequence{{Type: parsessac.SeqEmpty, Line: 3, Target: "unregistered"}},
