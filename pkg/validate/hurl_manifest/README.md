...
Hurl 테스트 entry 의 호출 흐름이 `manifest.yaml` `backend.auth` 모드 (cookie / bearer / none) 와 정합한지 교차 검증한다. 보호 구간 호출 전 인증 선행 여부 (XOH-06) 와 cookie 모드의 mutating 요청 CSRF 헤더 (XOH-07) 를 점검한다.
...
| XOH-07 | `xoh_07_csrf_on_mutation` | cookie 인증 모드에서 mutating 요청은 `X-CSRF-Token` 헤더 포함 (WARNING) | IF-ELSE | ✓ |
