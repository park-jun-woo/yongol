...
//ff:what cacheCfg — manifest.cache 섹션을 builtinBackend로 래핑 검증
...
func TestCacheCfg(t *testing.T) {
	cases := []struct {
		...
		{name: "nil_cache", fs: &yongol.Fullstack{Manifest: &pm.ProjectConfig{}}, wantPresent: false},
		{
			name:        "cache_present",
			fs:          &yongol.Fullstack{Manifest: &pm.ProjectConfig{Cache: &pm.BuiltinBackend{Backend: "postgres"}}},
			wantPresent: true,
