...
func TestXoh13CheckGuard_ZeroCov(t *testing.T) {
	...
	// non-guard type → nil.
	if got := xoh13CheckGuard(fn, ssac.Sequence{Type: "get"}, nil, nil); got != nil {
		t.Error("non-guard should return nil")
	}
	// eval with status 0 → nil (no default).
	if got := xoh13CheckGuard(fn, ssac.Sequence{Type: "eval"}, nil, nil); got != nil {
	...
	diags := xoh13CheckGuard(fn, seq, map[string]bool{}, nil)
	if len(diags) != 1 {
		t.Fatalf("expected 1 diag, got %d: %+v", len(diags), diags)
	}
	// covered → nil.
	if got := xoh13CheckGuard(fn, seq, map[string]bool{"404": true}, nil); got != nil {
	...
	if got := xoh13CheckGuard(fn, seq2, map[string]bool{}, nil); len(got) != 1 {
