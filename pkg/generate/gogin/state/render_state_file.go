...
// single statemachine. Emits only the <Symbol>Transitions map variable —
...
//
// id is the lowercase filename stem (used only in the source-reference
// comment). symbol is the PascalCase exported identifier — the only
// form spliced into Go code so consumers in other packages can reach
// the variable.
func renderStateFile(id, symbol string, transMap map[string]map[string]string) string {
	...
	b.WriteString(ffannot.EmitAnnotationBlock(ffannot.Block{
		...
		What: symbol + "Transitions — 상태 전이 테이블 (currentState, event → nextState)",
	}))
