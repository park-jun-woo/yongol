...
//ff:what requestValidatorSource — writeValidator가 기록하는 request_validator.go 템플릿 상수

package middleware

// requestValidatorSource is the verbatim Go source written to
// internal/middleware/request_validator.go. kin-openapi loads the embedded
// spec at startup and rejects requests whose payload violates schema
// constraints. Bundled as a const so it participates in normal Go build
// (no embedding needed) while leaving the generator function free of
// template noise.
const requestValidatorSource = `//` + `ff:func feature=runtime-middleware type=util control=sequence
