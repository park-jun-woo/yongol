...
//ff:what TestRenderArgValueLocation — FieldArg.Location 기반 TypeScript 식 생성 검증
...
func TestRenderArgValueLocation(t *testing.T) {
	tests := []struct {
		...
		{
			name: "LocVarSourceColumnDiffers",
			arg:  ir.FieldArg{Location: ir.LocVar, Key: "ID", ColumnName: "id", SourceColumn: "org_id", Source: "wf", Field: ".OrgID"},
			want: "wf.org_id",
		},
		{
			name: "LocVarNoField",
