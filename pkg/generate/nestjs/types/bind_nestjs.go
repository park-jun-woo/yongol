...
//ff:what bindNestJS — PGFamily + BindOpts → ir.TypeBinding 디스패치 (family 별 바인딩 위임)
...
// bindNestJS dispatches to the per-family binding functions based on the
// PGFamily classification. Each helper returns an ir.TypeBinding with
// Prisma schema type (DBType), TypeScript API type (APIType), and the
// conversion expressions between DB and API layers.
func bindNestJS(family typemap.PGFamily, opts ir.BindOpts) ir.TypeBinding {
	switch family {
