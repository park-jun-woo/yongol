//ff:func feature=report type=test control=iteration dimension=2 topic=sarif
...
// TestCollectResults_Nil covers the nil-report guard returning empty (non-nil)
// results and fired set.
...
// TestCollectResults_FilterAndFired covers nested iteration, the
// ERROR/WARNING-only filter (INFO skipped), and population of the fired set
// (only non-empty rule ids).
