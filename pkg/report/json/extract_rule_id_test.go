...
// TestExtractRuleID is table-driven, covering: simple id, compound id,
// no-prefix (original returned, empty id), and surrounding whitespace.
func TestExtractRuleID(t *testing.T) {
	cases := []struct {
		...
		{"compound", "[XOS-15] mismatch here", "XOS-15", "mismatch here"},
		{"no prefix", "plain message", "", "plain message"},
		{"empty", "", "", ""},
		{"bracket but not rule", "[lowercase-1] x", "", "[lowercase-1] x"},
		{"extra spaces trimmed", "[M-2]   trailing", "M-2", "trailing"},
	}
	for _, c := range cases {
