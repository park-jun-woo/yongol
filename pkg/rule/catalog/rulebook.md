...
| TM-59 | WARNING | `manifest.frontend.auth.refresh_field`가 선언되었지만 어떤 STML 액션도 `auth.refresh`를 캡처하지 않음 — login에서 refresh token이 저장되지 않아 무음 갱신이 동작하지 않음 (plans/gen/frontend Phase046, BUG-146) | `pkg/validate/stml_openapi/tm_59_refresh_field_capture.go` |
