...
export class CreateWorkflowVersionService {
  ...
  async createWorkflowVersion(params: any, user?: any): Promise<any> {
    return this.prisma.$transaction(async (tx) => {
      ...
      const newWf = await tx.workflow.create({ data: { org_id: wf.org_id, root_workflow_id: rootResult.root_workflow_id, title: wf.title, trigger_event: wf.trigger_event, version: versionResult.version } });
