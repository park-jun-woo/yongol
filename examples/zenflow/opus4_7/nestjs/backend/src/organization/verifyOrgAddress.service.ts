...
export class VerifyOrgAddressService {
  ...
  async verifyOrgAddress(params: any, body: any, user?: any): Promise<any> {
    return this.prisma.$transaction(async (tx) => {
      ...
      const geo = await this.geocodingService.geocode(params.address);
      await tx.organization.update({ where: { id: org.id }, data: { address_verified: geo.verified, latitude: geo.latitude, longitude: geo.longitude } });
