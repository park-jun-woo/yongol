...
//ff:checked llm=yongol-gen hash=a1ee40db
package service
...
func (server *Server) Login(ctx context.Context, request api.LoginRequestObject) (api.LoginResponseObject, error) {
	...
	token, err := auth.IssueToken(auth.IssueTokenRequest{Claims: model.UserClaim{Email: user.Email, ID: user.ID, Role: user.Role}})
	if err != nil {
