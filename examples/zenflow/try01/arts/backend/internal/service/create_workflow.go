...
//ff:checked llm=yongol-gen hash=065fd010
package service
...
func (server *Server) CreateWorkflow(ctx context.Context, request api.CreateWorkflowRequestObject) (api.CreateWorkflowResponseObject, error) {
	...
	workflow, err := qtx.WorkflowCreate(ctx, db.WorkflowCreateParams{OwnerID: currentUser.ID, Title: request.Body.Title, TriggerEvent: request.Body.TriggerEvent})
	if err != nil { return nil, err }
